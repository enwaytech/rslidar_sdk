...
robosense::lidar::SelfFilterSetup::inBufferedFootprint(const PointT& point)
{
  ...
  const double max_x = robot_params_.selfFilterCalibrationBaseLinkFrontOffset() + k_footprint_buffer_x_width_;
  const double min_x = robot_params_.selfFilterCalibrationBaseLinkBackOffset() - k_footprint_buffer_x_width_;
  const double max_y = (robot_params_.selfFilterCalibrationFootprintWidth() * 0.5) + k_footprint_buffer_y_width_;
